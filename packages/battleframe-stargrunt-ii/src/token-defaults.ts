...
/**
 * Default token images for SG2's actor subtypes. Registering a type->icon with
 * the engine's token-defaults registry (game.battleframe.tokens) means a fresh
 * unit Actor gets its shipped game-icon instead of Foundry's mystery-man -- the
 ...
 *
 * Types are module-id-namespaced (`battleframe-stargrunt-ii.unit`) and paths
 * point at this module's shipped `icons/` dir, both as Foundry v14 requires.
 * Mirrors the status.ts adoption pattern in this same module.
 */
...
export function stargruntDefaultTokenImages(): DefaultTokenImage[] {
  return [
...
/** Registers SG2's default token images via the engine's token registry at init. No-op without it. */
export function registerStargruntTokenDefaults(): void {
  const tokens = tokensApi();
  if (!tokens) {
    return;
  }
  for (const { type, img } of stargruntDefaultTokenImages()) {
