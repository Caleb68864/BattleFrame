...
/**
 * Missing i18n keys ship as raw strings while every other test stays green.
 * This asserts every literal `battleframe-incountry.*` key the code and
 * templates reference exists in en.json -- the scene-control titles and the
 * sheet label are full literals, exactly the kind that silently degrade to the
 * raw key (as the TYPES.Actor label did before it was added).
 */
...
describe("i18n completeness", () => {
  it("every literal battleframe-incountry key in src and templates exists in en.json", () => {
    ...
    for (const file of sources) {
      ...
      for (const match of text.matchAll(/["'](battleframe-incountry\.[a-zA-Z0-9_.]+)["']/g)) {
