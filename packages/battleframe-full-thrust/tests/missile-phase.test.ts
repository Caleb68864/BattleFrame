...
describe("advanceMissile", () => {
  it("moves the missile its full move up-range along course 12 (dead up) and ages it", () => {
    ...
    expect(next.y).toBeCloseTo(100 - requireRules().missileMoveMu * 2);
...
describe("missileExpired", () => {
  it("is not expired before its life span", () => {
    expect(missileExpired(missile({ turnsLived: requireRules().missileLifeTurns - 1 }))).toBe(false);
  });

  it("is expired at (and beyond) its life span", () => {
    expect(missileExpired(missile({ turnsLived: requireRules().missileLifeTurns }))).toBe(true);
    expect(missileExpired(missile({ turnsLived: requireRules().missileLifeTurns + 1 }))).toBe(true);
  });
});
